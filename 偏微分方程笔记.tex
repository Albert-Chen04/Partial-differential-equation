...
所以我们最终要解决的就是初始条件非齐次，方程和边界条件齐次\eqref{B}这样的方程.
	...
	\subsubsection{第二边界非齐次方程}
	对于有界方程非齐次问题（第二边界条件）：
	\begin{equation}
		\begin{cases}
			v_{tt} - a^2 v_{xx} = f_1(x, t), & 0 < x < l, \ t > 0, \\
			v|_{t=0} = 0, \quad v_t|_{t=0} = 0, & 0 \leq x \leq l, \\
			v_x|_{x=0} = 0, \quad v_x|_{x=l} = 0, & t \geq 0,
		\end{cases}
	\end{equation}
	若 \( w(x, t, \tau) \) 是以下定解问题的解：
	\begin{equation}
	...
	\end{equation}
	则函数 \( v(x, t) = \int_0^t w(x, t, \tau) \, d\tau \) 就是问题的解。
	同样地，方程的函数转移到最高偏导函数上，其余依旧为0.
	
	
	\subsubsection{无界非齐次方程}
	对于无界区域中的非齐次波动方程：
...
原函数初始条件求$a_n$
	...
	\end{equation}
		...
		\subsection{第二边值条件之分离变量}
求解过程一样，就是解不同，第一边界条件是把通解求出来，代入得系数，第二边界条件就还要求个导再代入。这样就不展开详细说明。
